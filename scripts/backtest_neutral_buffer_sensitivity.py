#!/usr/bin/env python3
"""
backtest_neutral_buffer_sensitivity.py
Teste plusieurs valeurs de BUFFER_NEUTRAL_PIPS (structure_analyzer.py,
actuellement 5 pips, jamais backteste) et mesure l'impact sur : (1) le
taux de temps passe en biais NEUTRAL (H1), et (2) le volume + Kelly des
trades SELL resultants, avec le reste du pipeline identique (confluence
H1/H4, buffer PD array 5 pips, MIN_RR_SELL=1.2, ATR>=3p).

Diagnostic du 17/08/2026 : le biais neutre H1 domine 26-57% du temps sur
les jours de semaine recents (vs 18-43% pour ATR faible), fort candidat
au vrai goulot principal de rarete des signaux actuel.
...
def analyze_structure(candles, buffer_neutral_pips, window_size=3):
...
def run_backtest(m5, h1, h4, pd_arrays, buffer_neutral_pips):
    LOOKBACK_H1 = 20 * 24
    LOOKBACK_H4 = 30 * 6

    trades = []
    n_neutral, n_total = 0, 0

    for i in range(LOOKBACK_H1, len(h1)):
        h1_window = h1[max(0, i - LOOKBACK_H1):i + 1]
        t_now = h1_window[-1]['candle_time']
        struct_h1 = analyze_structure(h1_window, buffer_neutral_pips, window_size=3)
        bias = struct_h1['bias']
        ...
        if len(h4_window) < 7:
            continue
        struct_h4 = analyze_structure(h4_window, buffer_neutral_pips, window_size=3)
    ...
    neutral_pct = round(100 * n_neutral / n_total, 1) if n_total else 0
    return trades, neutral_pct
...
if __name__ == '__main__':
    ...
    thresholds_pips = [0, 2, 5, 8, 12, 15]

    for pips in thresholds_pips:
        buffer_val = pips * 0.0001
        print(f"=== BUFFER_NEUTRAL_PIPS = {pips}p ===")
        trades, neutral_pct = run_backtest(m5, h1, h4, pd_arrays, buffer_val)
        metrics = compute_metrics(trades)
        print(f"  % de temps H1 neutre : {neutral_pct}%")
