#!/usr/bin/env python3
"""
backtest_choch_trigger.py
Quatrieme piste pour capter des mouvements que l'ATR M5 classique rate :
utiliser un CHoCH bearish confirme (structure de swings reelle, module
smc_choch_bos.py deja valide pour BUY au ticket #38) comme declencheur
alternatif a l'ATR, plutot qu'une figure chartiste visuelle (double top)
suggeree par une analyse externe mais non fondee sur les vraies donnees
du cas etudie (18/08 soir - verification a montre qu'il n'y avait pas de
vrai double top, juste une mauvaise lecture des etiquettes de prix).

Condition testee : signal accepte si ATR>=3p (comme actuellement) OU
CHoCH bearish + BOS bearish confirmes sur M5. Meme pipeline complet
sinon (confluence H1/H4, buffer 5 pips, MIN_RR_SELL=1.2, buffer neutre
2 pips).
...
def run_backtest(m5, h1, h4, pd_arrays, use_choch):
    LOOKBACK_H1 = 20 * 24
    LOOKBACK_H4 = 30 * 6
    trades = []
    n_added_by_choch = 0
    ...
    for i in range(LOOKBACK_H1, len(h1)):
        ...
        choch_ok = False
        if use_choch and not atr_ok and len(m5_before) >= 90:
            m5_window = m5_before[-90:]
            facts = smc_choch_bos.get_choch_bos_sweep_facts(m5_window, 'BEARISH')
            choch_ok = facts['choch_detected'] and facts['bos_confirmed']

        if not (atr_ok or choch_ok):
            continue
        ...
        if choch_ok and not atr_ok:
            n_added_by_choch += 1

        trades.append({'result': result, 'rr_real': rr_real})

    return trades, n_added_by_choch
...
if __name__ == '__main__':
    ...
    trades_a, _ = run_backtest(m5, h1, h4, pd_arrays, use_choch=False)
    for k, v in compute_metrics(trades_a).items():
        print(f"  {k}: {v}")

    print("\n=== ATR OU CHoCH+BOS bearish confirmes M5 ===")
    trades_b, added_b = run_backtest(m5, h1, h4, pd_arrays, use_choch=True)
    for k, v in compute_metrics(trades_b).items():
        print(f"  {k}: {v}")
    print(f"  dont setups ajoutes uniquement par le CHoCH+BOS : {added_b}")
