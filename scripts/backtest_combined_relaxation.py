#!/usr/bin/env python3
"""
backtest_combined_relaxation.py
Suite a la question du 20/08 sur la superposition de conditions : teste
la combinaison des DEUX seuls leviers ayant deja une preuve empirique
solide de rester profitables en volume accru (contrairement a retirer la
confluence H1/H4, qui n'apporte que +13.9% de volume, backtest #42) :

1. Buffer PD array elargi (5p actuel -> 8p/10p, backtest #39 montrait
   deja Kelly +0.369 a 8p vs -0.126 a 5p, jamais pousse plus loin)
2. Killzone retiree (24h/24, backtest #45 montrait Kelly +0.247 hors
   killzone vs +0.703 en killzone - degrade mais reste positif)

Meme pipeline sinon (confluence H1/H4 maintenue, MIN_RR_SELL=1.2, ATR>=3p,
buffer neutre 2p).
...
def in_killzone(dt):
    if dt.weekday() >= 5:
        return False
    h = dt.hour
    return any(start <= h < end for start, end in KILLZONES_UTC)
...
def try_build_sell_signal(current_price, active_arrays, struct, array_buffer):
...
def run_backtest(m5, h1, h4, pd_arrays, array_buffer, require_killzone):
    LOOKBACK_H1 = 20 * 24
    LOOKBACK_H4 = 30 * 6
    signals = []
    blocked_until = None  # Contrainte "une seule position a la fois",
    # comme un vrai compte reel - aucun nouveau signal evalue tant que le
    # precedent n'est pas resolu (SL ou TP touche).
    ...
    for i in range(RECENT_LOOKBACK, len(m5)):
        ...
        sig = try_build_sell_signal(current_price, active_arrays, struct_h1, array_buffer)
        if not sig:
            continue
        ...
        # Trouver le moment exact de resolution pour bloquer les signaux suivants
        resolved_at = None
        ...
        signals.append({**sig, 'time': t_now, 'result': result, 'rr_real': rr_real})
...
if __name__ == '__main__':
    conn = get_conn()
    START = '2026-08-18'  # verification 21/08 : restreint a la periode
    # POST-FIX bougie H1/H4 live (ticket #56, deploye le 18/08) - avant
    # cette date, le bias H1/H4 stocke en base pouvait etre fige jusqu'a
    # 59 min/heure, creant un edge artificiel dans le backtest absent de
    # la vraie production historique.
    ...
    configs = [
        ("BASELINE (buffer 5p, killzone active) - POST-FIX uniquement", 0.00050, True),
    ]

    for label, buf, kz in configs:
        print(f"=== {label} ===")
        trades = run_backtest(m5, h1, h4, pd_arrays, buf, kz)
