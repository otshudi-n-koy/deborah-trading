#!/usr/bin/env python3
"""
backtest_min_sl_sensitivity.py
Teste plusieurs valeurs de MIN_SL (plancher de risque minimum applique
quand la zone PD array matchee est trop etroite, actuellement 10 pips,
jamais backteste) et mesure l'impact sur volume + Kelly des trades SELL.

Meme pipeline complet (confluence H1/H4, buffer PD array 5 pips,
MIN_RR_SELL=1.2, ATR>=3p, buffer neutre 2 pips - ticket #54).
...
def try_build_sell_signal(current_price, active_arrays, struct, min_sl):
...
def run_backtest(m5, h1, h4, pd_arrays, min_sl):
    LOOKBACK_H1 = 20 * 24
    LOOKBACK_H4 = 30 * 6
    trades = []
    for i in range(LOOKBACK_H1, len(h1)):
        ...
        sig = try_build_sell_signal(current_price, active_arrays, struct_h1, min_sl)
        if not sig:
            continue
...
if __name__ == '__main__':
    ...
    for pips in [3, 5, 8, 10, 15, 20]:
        min_sl = pips * 0.0001
        print(f"=== MIN_SL = {pips}p ===")
        trades = run_backtest(m5, h1, h4, pd_arrays, min_sl)
