#!/usr/bin/env python3
"""
backtest_tp_extension.py
Teste un TP alternatif par projection etendue ("measured move" ICT) :
au lieu de viser uniquement liquidity_target/swing_low (souvent un niveau
proche, limite le RR), on projette une cible = entry - (amplitude du range
H1) * multiplicateur, et on prend le TP le PLUS ELOIGNE entre cette
projection et le liquidity_target actuel (jamais moins ambitieux que
l'existant, seulement potentiellement plus).

Origine: piste alternative suite a l'echec du filtre "deplacement net"
(ticket #58) pour capter le cas du 18/08 soir. Le vrai facteur limitant
identifie etait le RR insuffisant faute de cible assez ambitieuse - cette
approche s'attaque directement au TP plutot qu'au declencheur d'entree.

Meme pipeline complet (confluence H1/H4, buffer 5 pips, MIN_RR_SELL=1.2,
ATR>=3p, buffer neutre 2 pips), seul le calcul du TP change.
...
def try_build_sell_signal(current_price, active_arrays, struct, tp_multiplier):
    ...
    tp_baseline = swing_low if liq_target > current_price - 0.0015 else liq_target
    if tp_baseline >= current_price:
        tp_baseline = liq_target

    if tp_multiplier > 0:
        htf_range = abs(swing_high - swing_low)
        tp_projection = round(entry - htf_range * tp_multiplier, 5)
        tp = min(tp_baseline, tp_projection)
    else:
        tp = tp_baseline
...
def run_backtest(m5, h1, h4, pd_arrays, tp_multiplier):
    LOOKBACK_H1 = 20 * 24
    LOOKBACK_H4 = 30 * 6
    trades = []
    for i in range(LOOKBACK_H1, len(h1)):
        ...
        sig = try_build_sell_signal(current_price, active_arrays, struct_h1, tp_multiplier)
        if not sig:
            continue
...
if __name__ == '__main__':
    ...
    for mult in [0, 0.5, 1.0, 1.5, 2.0]:
        label = "BASELINE (TP actuel)" if mult == 0 else f"TP etendu x{mult} du range H1"
        print(f"=== {label} ===")
        trades = run_backtest(m5, h1, h4, pd_arrays, tp_multiplier=mult)
