#!/usr/bin/env python3
"""
backtest_buy_reactivation_sensitivity.py
Chantier #70 - etape 1 : backtest BUY avec le pipeline COMPLET actuel
(confluence H1/H4, buffer PD array 5 pips, ATR>=3p, buffer neutre 2p),
SANS le filtre CHoCH+BOS, sur toute la periode disponible.
...
def try_build_buy_signal(current_price, active_arrays, struct, min_rr):
...
def run_backtest(m5, h1, h4, pd_arrays, min_rr, require_confluence):
    LOOKBACK_H1 = 20 * 24
    LOOKBACK_H4 = 30 * 6
    trades = []
    blocked_until = None
    ...
    for i in range(RECENT_LOOKBACK, len(m5)):
        ...
        sig = try_build_buy_signal(current_price, active_arrays, struct_h1, min_rr)
        if not sig:
            continue
...
if __name__ == '__main__':
    ...
    print("=== BUY sans confluence H4 (H1 seul, comme le shadow actuel) ===")
    for min_rr in [1.0, 1.2, 1.5]:
        print(f"--- RR>={min_rr} ---")
        trades = run_backtest(m5, h1, h4, pd_arrays, min_rr, require_confluence=False)
        for k, v in compute_metrics(trades).items():
            print(f"  {k}: {v}")

    print("\n=== BUY AVEC confluence H1+H4 (plus strict, comme SELL en prod) ===")
    for min_rr in [1.0, 1.2, 1.5]:
        print(f"--- RR>={min_rr} ---")
        trades = run_backtest(m5, h1, h4, pd_arrays, min_rr, require_confluence=True)
        for k, v in compute_metrics(trades).items():
            print(f"  {k}: {v}")
