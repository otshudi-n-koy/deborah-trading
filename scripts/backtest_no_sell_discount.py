#!/usr/bin/env python3
"""
backtest_no_sell_discount.py
Teste a grande echelle la regle B decouverte le 25/08 sur 9 vrais trades
(6/9 pertes evitees) : interdire le SELL si l'entree se situe en zone
DISCOUNT (sous l'equilibre H1, range_mid = (swing_high+swing_low)/2).

Rationnel SMC : en zone discount, un mouvement baissier est deja
"epuise" par definition - un SELL n'a normalement de sens qu'en zone
PREMIUM (au-dessus de l'equilibre).

Meme pipeline complet que tous les backtests de la semaine (confluence
H1/H4, buffer PD array 5 pips, MIN_RR_SELL=1.2, ATR>=3p, buffer neutre
2p), seule la nouvelle condition premium/discount varie.
...
def analyze_structure(candles, window_size=3):
    if len(candles) < window_size * 2 + 1:
        return {'bias': 'NEUTRAL', 'swing_high': None, 'swing_low': None,
                'liquidity_target': None, 'range_mid': None}
    ...
    if len(swing_highs) < 2 or len(swing_lows) < 2:
        ...
        return {'bias': bias, 'swing_high': fH, 'swing_low': fL,
                'liquidity_target': fL if bias == 'BEARISH' else fH, 'range_mid': eq50}
    ...
    return {'bias': bias, 'swing_high': last_hh['price'], 'swing_low': last_ll['price'],
            'liquidity_target': liq, 'range_mid': range_mid}
...
def run_backtest(m5, h1, h4, pd_arrays, require_premium):
    LOOKBACK_H1 = 20 * 24
    LOOKBACK_H4 = 30 * 6
    trades = []
    n_blocked_by_discount = 0
    blocked_until = None  # Contrainte "une seule position a la fois", comme
    # un vrai compte reel - CORRECTION du 25/08 : cette contrainte manquait
    # dans la premiere version de ce script (meme bug que celui trouve et
    # corrige le 20/08 dans backtest_combined_relaxation.py, n=42 gonfle a
    # n=13 une fois corrige).

    for i in range(RECENT_LOOKBACK, len(m5)):
        t_now = m5[i]['candle_time']

        if blocked_until is not None and t_now <= blocked_until:
            continue
        ...
        atr_before = [c for c in m5 if c['candle_time'] <= t_now][-14:]
        if compute_atr_pips(atr_before, 14) < ATR_MIN_PIPS:
            continue
        ...
        if require_premium:
            range_mid = struct_h1['range_mid']
            if range_mid is not None and sig['entry'] < range_mid:
                n_blocked_by_discount += 1
                continue

        if trades and trades[-1]['pd_array_id'] == sig['pd_array_id']:
            continue

        m5_after = m5[i + 1:i + 2001]
        result, rr_real = simulate_trade(sig['entry'], sig['sl'], sig['tp'], m5_after)
        if result is None:
            continue

        resolved_at = None
        for c_after in m5_after:
            if c_after['high'] >= sig['sl'] or c_after['low'] <= sig['tp']:
                resolved_at = c_after['candle_time']
                break
        blocked_until = resolved_at

        trades.append({**sig, 'time': t_now, 'result': result, 'rr_real': rr_real})

    return trades, n_blocked_by_discount
...
if __name__ == '__main__':
    ...
    print("=== BASELINE (comportement actuel, SELL autorise en discount) ===")
    trades_base, _ = run_backtest(m5, h1, h4, pd_arrays, require_premium=False)
    for k, v in compute_metrics(trades_base).items():
        print(f"  {k}: {v}")

    print("\n=== REGLE B : SELL interdit en zone DISCOUNT ===")
    trades_rule, n_blocked = run_backtest(m5, h1, h4, pd_arrays, require_premium=True)
    for k, v in compute_metrics(trades_rule).items():
        print(f"  {k}: {v}")
    print(f"  Setups bloques par la regle (auraient ete pris sinon): {n_blocked}")
